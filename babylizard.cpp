...
public:
    ...
    void UpdateAttackZone(Lizard &lz) {
        ...
        int min;
        int max;
        if (CountOfLizardsAdded < P) { // Calculate the attack zone of the just added lizard only if the problem is not solved yet
...
//    clock_t start = clock();
    ...
    for (int i = 0; i < g.N * g.N; i++) {
        ...
        if (g.input[i] == 2) { // We have encountered a tree. If there is no tree to its immediate left, create a new segment. This will also take care of left edge because of the way we are setting previousTreeIndex above. In any case, reset previousTreeIndex to current index.
